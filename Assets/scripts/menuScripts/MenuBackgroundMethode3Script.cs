...
public class MenuBackgroundMethode3Script : MonoBehaviour {
    ...
    [HideInInspector]
	public PanelMethode3Script[,] positions; // les positions
	...
	void Start () {
		...
		positions = new PanelMethode3Script[nbX, nbY];

		// Puis on ajoute tous les cubes ! <3
		for(int i = 0; i < nbCubes; i++) {
			// On instancie notre cube
			GameObject monCube = Instantiate(cubePrefabs) as GameObject;
            monCube.GetComponent<PanelMethode3Script>().menu = this;

			// On lui donne cette position
			int x = i / nbY;
			int y = i % nbY;
            monCube.GetComponent<PanelMethode3Script>().x = x;
            monCube.GetComponent<PanelMethode3Script>().y = y;
			positions[x, y] = monCube.GetComponent<PanelMethode3Script>();
	...
	public PanelMethode3Script getPanelXY(int x, int y ) {
