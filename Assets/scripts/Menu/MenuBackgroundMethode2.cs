...
public class MenuBackgroundMethode2 : MonoBehaviour {
	...
	void Start () {
		...
		for(int i = 0; i < nbCubes; i++) {
            ...
            monCube.GetComponent<PanelMethode2>().menu = this;

			// On lui donne cette position
			int x = i / nbY;
			int y = i % nbY;
            monCube.GetComponent<PanelMethode2>().x = x;
            monCube.GetComponent<PanelMethode2>().y = y;
	...
	public PanelMethode2 getPanelXY(int x, int y ) {
		foreach (GameObject g in cubes)
		{
			PanelMethode2 p = g.GetComponent<PanelMethode2>();
			if(p.x == x && p.y == y) {
				return p;
			}
		}
