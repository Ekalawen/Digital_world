...
public class OnlyFloorCubeMap : CubeMap {
    ...
    protected override void GenerateCubeMap() {
        ...
        Mur floor = new Mur(Vector3.zero, Vector3.right, tailleMap.x, Vector3.forward, tailleMap.y);
    ...
    protected override Vector3 GetPositionCave(Vector3Int sizeCave) {
        if (caveOffsetSides) {
            ...
            return new Vector3(Random.Range(0, tailleMap.x - sizeCave.x + 1),
                Random.Range(1, tailleMap.y - sizeCave.y + 1), // On autorize pas la cave à être complètement au sol !
                Random.Range(0, tailleMap.z - sizeCave.z + 1));
        }
    }
}
