...
public class LumiereFurtive : Lumiere {
    ...
    public enum EtatLumiereFurtive { HIDDEN, VISIBLE };
    ...
    protected EtatLumiereFurtive etat;
    ...
    protected void UpdateSpecific() {
        ...
        switch (etat) {
            case EtatLumiereFurtive.HIDDEN:
            ...
            case EtatLumiereFurtive.VISIBLE:
    ...
    protected EtatLumiereFurtive UpdateEtat() {
        if(IsInPlayerSight(transform.position, player)) {
            if (etat != EtatLumiereFurtive.VISIBLE) {
                //    gm.soundManager.PlayGetLumiereClip(transform.position);
                posObjectifVisible = transform.position;
            }
            etat = EtatLumiereFurtive.VISIBLE;
            return EtatLumiereFurtive.VISIBLE;
        } else {
            if (etat != EtatLumiereFurtive.HIDDEN) {
                //    gm.soundManager.PlayGetLumiereClip(transform.position);
                posObjectifHidden = transform.position;
            }
            etat = EtatLumiereFurtive.HIDDEN;
            return EtatLumiereFurtive.HIDDEN;
        }
    }
