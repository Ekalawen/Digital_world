...
public class InputControllerQWERTY : InputController {

    public override Vector3 GetCameraSelectorMouvement() {
        float x = inputManager.GetAxis("CameraSelector_X_QWERTY");
        float y = inputManager.GetAxis("CameraSelector_Y");
        float z = inputManager.GetAxis("CameraSelector_Z_QWERTY");
        return new Vector3(x, y, z);

    }

    public override Vector3 GetHorizontalMouvement(bool rawAxis = false) {
        float x = inputManager.GetAxis("HorizontalQWERTY", rawAxis);
        float y = 0;
        float z = inputManager.GetAxis("VerticalQWERTY", rawAxis);
        return new Vector3(x, y, z);
    }
